...
export class AdminGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext) {
        ...
        // admin 검증
        const { role } = await this.userService.adminCheck(userId);
        ...
        */
        if (
            ...
            request.auth = { isLoggedIn: false, userId: null, isAdmin: false };
        ...
        if (role !== 1) {
            request.auth = { isLoggedIn: true, userId, isAdmin: false };

            return true;
        }

        request.auth = { isLoggedIn: true, userId, isAdmin: true };
